...
alert('Hello'); alert("Hi") // this line is not having semicolon
...
alert(3 +
1
+ 2); // For integer number type we do no need to use `` for multi line 
...
while (countdown > 0) {
 ...
 if(countdown == 100) // Here i made a change
  {
  ...
  var msg = "";
  if (lemein) {   // Were the string ("0") inside the condition become true 
   msg += "hi";    //  Hence this block gets activated
   }
  if (lemeout) {  // Were the Integer (0) inside the condition become False. 
   msg += "Hello";
  }
